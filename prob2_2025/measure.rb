...
closed wrt countable union, intersection and complementation. Can
 we really do this? The short answer is "Yes". 
...
Let's do the same thing now starting with two sets <M>A,B\seq\Omega.</M>  Convince yourself that you can generate
a collection of 16 sets starting with these.
If you are confused, then look at the following diagram.
<CIMG web="caravenn.png"/>
 I again ask you
 the same question: if I tell you the lengths of <M>A</M>  and <M>B</M>, can you find out the lengths of all these? Now,
...
Let <M>\Omega</M>  be a nonempty set and <M>\calF</M>  be a collection of its subsets, closed
 under (finite) intersection with <M>\Omega\in\calF.</M>  . We
 have a function <M>\lambda:\calF\to[0,\infty)</M>  that is countably additive, and <M>\lambda(\Omega) < \infty.</M> Then we can
 uniqely extend it to <M>\sigma(\calF).</M>
</THM>
