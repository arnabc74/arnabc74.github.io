...
The only point that one feels uncomfortable about is the choice of <M>A.</M>  in the univariate case, obtaining <M>\sigma</M> 
 from <M>\sigma^2</M>  was straightforward, just take the unique nonnegative square root. But in the multivariate set up
 there are, ingeneral infinitely many choices for <M>A</M>  such that <M>\Sigma=AA'.</M>  Which one should we take. Fortunately
 it dows not matter here, because as we are going to show now, any choice leads to the same distribution of <M>A\v X+\v\mu.</M> 
 
For this purpose we shall use characteristic functions. In the multivariate set up, they are defined as follows.
...
Now let us return to the definition multivariate normal. We shall take any <M>A</M>  with
