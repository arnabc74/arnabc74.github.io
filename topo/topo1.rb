...
<U>(If part)</U>  Let <M>G</M> be closed in <M>(X,\tau).</M>  But since <M>G\seq Y,</M>  hence   <M>G = G\cap Y.</M>  So <M>G</M> 
...
Since <M>x\not\in A',</M>  hence <M>\exists U\in\tau~~x\in U\mbox{ and }U\setminus\{x\}\cap A=\phi.</M>  
...
\{n,n+1,n+2,...\}.</M> Show that the
collection <M>\{A_n~:~n\in\nn\}\cup\{\phi\}</M> is a topology on <M>\nn.</M>[3]
</E>@}</EBODY><SOLN/>ei a.nkaTA Agei Alochita hayechhe. ekhAne
AbAr dilAm, kAraN nIcher a.nkaTA eTAr bhittite deoyA hayechhe.</EXM>
...
@{<WR>
Here the open sets are of the form <M>B_n = \{1,2,...,n\}</M>  for <M>n\in\nn.</M>   

A point <M>p\in\nn</M>  is in the derived set if and only if all open neighbourhoods of he <M>p</M>  intersect <M>A</M> 
 at some point other than <M>p.</M>  

So the required derived set is <M>\{1948,1949,...\} = A_{1948}.</M>
</WR>@}
<DSQS/></EXM>
