...
that <M>Z</M> has <B>Bern</B>(0.1) distribution. Hence compute <M>Var(Z)</M>. 
<P/>
...
[Thanks to Tushti for corrected a typo.]
</EXM>
...
::<EXR>(Easy)<EIMG web="dtwo1.png"/><ANS>
<M>[[P(X=i)][P(X=i-1)]] = [[7-i][i]]</M>  for <M>i=1,...,6</M>. This is <M>\gtreqless 1</M>  according as 
<M>3 \gtreqless i</M>.</ANS></EXR>
::<EXR>(Medium)<EIMG web="dtwo2.png"/><ANS>
See the hint of the last problem.
</ANS></EXR>
::<EXR>(Medium)<EIMG web="dtwo3.png"/><ANS>
Model the situation as follows. Each passenger independently flips a coin with <M>P(head) = 0.95</M>. If tail, they cancel
 their booking.  

Let <M>X = </M>  number of passengers showing up. 

Then <M>X\sim Binom(52, 0.(5)</M>. We want to find <M>P(X\leq 50) = 1-P(X=51)-P(X=52) = \cdots</M>.
</ANS></EXR>
::<EXR>(Medium)<EIMG web="dtwo4.png"/><ANS>
The number of ways to write a string of <M>x_1</M>  first outcomes, <M>x_2</M>  second outcomes and so on, is 
<D>[[n!][x_1!\cdots x_r!]]</D>.
Each such outcome has probability <M>p_1^{x_1}\cdots p_r^{x_r}</M>.
</ANS></EXR>
::<EXR>(Medium)<EIMG web="dtwo5.png"/>

Here <M>1\leq k < r</M>. <ANS>
Consider the event <M>A</M>  defined as {one of the first <M>k</M>  outcomes has occured}. 

Then <M>X_1+\cdots+X_k = </M>  number of times <M>A</M>  has occured out of <M>n</M>  trials. Also <M>P(A)= p_1+\cdots+p_r</M>.

Hence <M>X_1+\cdots+X_k \sim Binom(n,p_1+\cdots+p_r)</M>.

Now you can write down the required PMF esily. 
</ANS>
</EXR>
::<EXR>(Medium)<EIMG web="dtwo6.png"/><ANS>
Model the situation as follows. Each customer rolls a 3 faced die. The three faces are "Ordinary", "Colour" and "Browse". The probabilities
 are <M>0.5, 0.3, 0.2</M>, respectively.  The rolls are independent. 

Let <M>X_1, X_2, X_3</M>  be the numbers of customers (out of 5) in each category. 

So the required probability is <M>[[5!][1!2!2!]] (0.5)^1(0.2)^2(0.3)^2</M>.
</ANS></EXR>
::<EXR>(Medium)<EIMG web="dtwo7.png"/></EXR>
::<EXR>(Medium)<EIMG web="dtwo8.png"/><ANS>
Toss a coin with <M>P(head)=p</M>  independently until you get <M>r</M>  heads. Let <M>X</M>  be the number of tails obtained.
 
</ANS></EXR>
::<EXR>(Hard)<EIMG web="dtwo9.png"/>
<ANS><B>First part:</B> Let us start by considering one possible sequence of results that will require exactly 7 games: AAABBBB.
 But BBBBAAA is not possible, because here the decision is already clear after the fourth game. So we need a sequence of
 length 7 such that there are exactly 4 of the last symbol. Each such sequence must have exactly 3 of the other symbol. Each
 such sequence ending with A has probability <M>p^4(1-p)^3</M>  while each such sequence ending with B has probability <M>p^3(1-p)^4</M>.
 
All that remains is to count how many sequences of these two types there are. It is an easy calculus exercise to show that
 the maximum occurs at <M>p=[[12]]</M>. 

<ANS><B>Second part:</B></ANS>  If <M>i=2</M>, the decision must be made after playing at most 3 games (pigeon hole) and
 at least 2 games. Find the probability of each as in the first part. The case <M>i=3</M>  is similar.
</ANS>
</EXR>
::<EXR>(Medium)<EIMG web="dtwo10.png"/><ANS>
Let <M>X,Y</M>  be, respectively, the number of heads obtained by <M>A,B</M>. Then <M>X\sim Binom(k,0.5)</M>  and <M>Y\sim Binom(n-k,0.5)</M>.
 Also they are independent. Also <M>X+Y\sim Binom(n,0.5)</M>.

<MULTILINE>
P(X = Y) 
& = & \sum_{i=0}^k P(X=i,\, Y=i)\\
& = & \sum_{i=0}^k P(X=i)P(Y=i)<SINCE><M>\because X,Y</M>  independent</SINCE>\\
& = & 2^{-n}\sum_{i=0}^k \binom k i \binom {n-k} i<SINCE><M>\because X,Y</M>  independent</SINCE>\\
</MULTILINE> 
Also <M>P(X+Y=k) = \binom n k 2^{-n}</M>.

So we need to show <M>\sum_{i=0}^k \binom k i \binom {n-k} i = \binom n k</M>.

Now <M>\sum_{i=0}^k \binom k i \binom {n-k} i = \sum_{i=0}^k \binom k {k-i} \binom {n-k} i = \binom {k+(n-k)} {(k-i)+i} =\binom n k</M>.
</ANS></EXR>
::<EXR>(Hard)<EIMG web="dtwo11.png"/><ANS>
(b) Let <M>A = \{\underbrace{TT...T}_kH~:~k\in\nn\}</M>  and <M>B =
...
::<EXR><EIMG web="dtwo25.png"/></EXR>
::<EXR><EIMG web="dtwo26.png"/></EXR>
::<EXR><EIMG web="dtwo27.png"/></EXR>
::<EXR><EIMG web="dtwo28.png"/></EXR>
::<EXR><EIMG web="dtwo29.png"/></EXR>
::<EXR><EIMG web="dtwo30.png"/></EXR>
::<EXR><EIMG web="dtwo31.png"/></EXR>
::<EXR><EIMG web="dtwo32.png"/>

Here Exercise 16 refers to the last problem. 
</EXR>
::<EXR><EIMG web="dtwo33.png"/></EXR>
::<EXR><EIMG web="dtwo34.png"/></EXR>
::<EXR><EIMG web="dtwo35.png"/></EXR>
::<EXR><EIMG web="dtwo36.png"/></EXR>
