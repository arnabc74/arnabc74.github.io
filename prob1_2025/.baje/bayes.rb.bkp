...
<ANS><M>p_3p_6 + p_4p_5+p_5p_4+p_6p_3.</M>  [Thanks to Anant Raj for correcting a mistake here.]</ANS>
</EXR>
...
Here <M>|A|=4\times 48\times\cdots\times(48-19+1)\times 32!</M>, 

<M>|A\cap B| = 1\times 3\times
 48\times\cdots\times(48-19+1)\times31!</M>  and 
<M>|A\cap C| = 1\times 4\times 47\times\cdots\times(47-19+1)\times31!.</M>
...
If you are not convinced by this argument, you may proceed rigourously as follows. Here are
 all  possible second draw strategies: <OL><LI>always WR</LI><LI>always WOR</LI><LI>WR if first is R, WOR otherwise</LI><LI>WOR if first is R, WR otherwise.</LI></OL>
...
<D>P_1(\mbox{correct}) = \sum_X P_1(X)P_1(\mbox{correct}|X),</D>
where the sum is over <M>X\in\{</M> RR, RB, BR, BB<M>\}</M>. Here I am writing <M>P_1</M>  to mean
 probability under strategy 1.  <M>P_1(RR) = [[12]]\times [*[ (*([[23]])*)^2 + (*([[101][201]])*)^2 ]*]</M>. Also we shall
 guess urn A for outcome RR. So <M>P_1(\mbox{correct}|RR) = [[(*([[2][3]])*)^2][ (*([[2][3]])*)^2 + (*([[101][201]])*)^2]]</M>.
