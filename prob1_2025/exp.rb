...
::<EXR>(Easy)A random variable <M>X</M>  takes the values <M>-2, -1, 0, 1 </M>  and <M>2</M>  with
 probabilities <M>p,q,1-2p-2q, p</M>  and <M>q,</M>  respectively. Find <M>E(X).</M></EXR>

::<EXR>(Easy)A random variable takes the values <M>1,2,...,10</M>  with probabilities
...
<EXR>(Easy)Which of the following must be true?
<HL>
<LI><M>X = X_++X_-.</M></LI>
<LI><M>X = X_+-X_-.</M></LI>
<LI><M>X = X_--X_+.</M></LI>
...
<EXR>(Easy)Prove this for simple <M>X</M>.</EXR>
...
::<EXR>(Medium)Which is larger <M>(E(X))^2</M> or <M>E(X^2)?</M> Assume
...
::<EXR>(Medium)If <M>E(|X|)=\infty,</M>  then what can you say about <M>E(X)?</M>
...
::<EXR>(Medium)Show that if <M>X</M>  is a random variable taking only non-negative
integer values, then 
...
::<EXR>(Medium)For a group of <M>n</M>  people find the expected number of days of the year which are
...
::<EXR>(Medium)A man with <M>n</M>  keys wants to open a door (where exactly one key works). He tries the
...
::<EXR>(Medium)<CIMG web="rossrv7.png"/>
<ANS>
...
::<EXR>(Medium)<CIMG web="rossrv12.png"/><ANS>
...
::<EXR>(Easy)<CIMG web="rossrv13.png"/>
<ANS>
<M>E(X) = [[52]].</M>
</ANS>
</EXR>
::<EXR>(Medium)<CIMG web="rossrv14.png"/>
...
::<EXR>(Medium)<CIMG web="fellrv2.png"/>
<ANS>
