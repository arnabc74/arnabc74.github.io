...
By differentiating we may obtain the pdf of the <M>i</M>-th order
statistic as:
<D>
...
with <M>[[S_k][(S_{n+1}-S_k)+S_k]].</M> So dividing both <M>S_k</M>
...
& = & Z_n\times \sqrt{[[k][n+1]]} +  
[[k][\sqrt{n+1}]] - p\sqrt{n+1}\tod N(0,p),
</MULTILINE>
since <M>\sqrt{[[k][n+1]]}\to \sqrt p</M> and <M>[[k][\sqrt{n+1}]] - p\sqrt{n+1}\to 0.</M> 
...
Let <M>T_1, T_2,...</M> be a sequence of 
...
By MVT, <M>g(T_n)-g(\mu) = \nabla g(\xi_n)(T_n-\mu)</M> for
some <M>\xi_n</M> on the segment joining <M>\mu </M> and <M>T_n.</M>
<Q>
<B>[Because:</B>
<Q>
