...
<HEAD1 u="https://youtu.be/OCNS43-6rzs">Borel <M>\sigma</M>-algebra</HEAD1>
First, let
...
<HEAD1 u="https://youtu.be/awTpjHeyXOw">Caratheodory extension</HEAD1>
...
Let <M>\Omega=[0,10]</M>  which has length <M>10</M>. Also let <M>A,B</M>  be two intervals in <M>\Omega</M>  each having
...
<HEAD1>A smarter way to write</HEAD1>
While the "partition the codomain" idea is intuitively very nice, there is notationally a much
 neater way to arrive at the definition of Lebesgue integral. It is this definition that we will
 see most often in books, and is very similar to the definition of Riemann  integration in appearance. 

 Let <M>\calS</M>  denote the set of all
 nonnegative simple measurable functions   on <M>[a,b].</M>  Then we define, for <M>s=\sum_1^n c_i
 1_{A_i} \in\calS,</M>
  its Lebesgue integral as the area under it:
<D>\int s\, d \lambda = \sum_1^n c_i \lambda(A_i).</D> 
Then for a general measurable <M>f:[a,b]\to[0,M]</M>  we consider
<D>\sup{*{\int s\, d \lambda~:~s\in\calS,~~s\leq f }*}</D>
and
<D>\inf{*{\int s\, d \lambda~:~s\in\calS,~~f\leq s }*}.</D>
It may not be immediately apparent that the sup will be same as what we had called sup(red area). SImilarly, the inf actually
 equals our inf(blue area). 

If this appears to be trivial to you, consider the following diagram:
<CIMG web="reform1.png"/>
This <M>s\leq f</M>  is in <M>\calS.</M>  But it cannot occur as the lower approximation by 
 partitioning the codomain. However, this is "too far below <M>f</M>" and will never contribute to
 the sup, as it gets beaten by <M>s'</M>  shown below. 
<CIMG web="reform2.png"/>
    This <M>s'</M>  is obtained by the partition codomain technique using the values of <M>s</M>  above. Thus, for any finite
 set of values, the "best candidate" comes from the "partition the codomain" approach. 
...
For this it is enough to show that 
 <M>\forall \delta>0~~\lim \int s_n\, dP \geq \int z\, dP- \delta.</M>  
<BECAUSE>
...
We are showing: 
<Q><M>\forall</M> simple measurable <M>0\leq z\leq f~~\forall \delta>0~~\lim \int s_n\, dP \geq
 \int z\, dP- \delta.</M>  </Q>
...
Then we show that for <M>a>0</M>  we have <M>E(aX) = aE(X).</M>  This proof also proceeds in three steps (left as an exercise).
