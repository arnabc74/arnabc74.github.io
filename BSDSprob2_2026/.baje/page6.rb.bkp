...
<HEAD2>Box-Cox transform</HEAD2>
This is a family of variance stabilizing transforms that have an extra benefit: they also correct
 skewness in data. 

<DEFN name="Box-Cox transform">
For <M>\lambda\in\rr</M>  we define the <TERM>Box-Cox transform</TERM>  as a function <M>f_ \lambda:\rr\to\rr</M>  as
<D>f_ \lambda(x) = <CASES>[[x^ \lambda-1][\lambda]]<IF>\lambda\neq 0</IF> \log x<IF>\lambda=0</IF></CASES>.</D>
</DEFN> 
Notice that for each <M>\lambda\in\rr</M>
 you have one transform. <LINK to="#box">This exercise</LINK> in the problem set below will give
...
But arguably its "skew-correcting role" is more helpful in practice. Many statistical methods are designed with normal data
 ...
 skewness. Then we can play with  <M>\lambda</M>  to come up with an optimal value for which the
 ...
 often a simpler alternative. You may have a taste of this with simulated data using <LINK to="https://acbsds.shinyapps.io/boxcox/">this
 interactive demo</LINK>. There is a slider to control the value of <M>\lambda.</M>  The initial value is <M>\lambda=1,</M> 
 which basically gives the shape of the histogram of the raw data. It is highly skewed. Play with the slider, and see how
 the histogram changes shape. Try to find <M>\lambda</M>  for which the histogram is nearly symmetric.
...
<EXR>Let <M>X\sim</M> Exponential distribution with rate <M>\lambda</M>.  Find a variance
 stabilizing transform for <M>X.</M></EXR>
<DEST name="box"><EXR>Let <M>X</M> satisfy <M>V(X) = k E(X)^ \alpha.</M>, where <M>k</M>  and <M>\alpha</M>
  are known constants.
Find a variance
 stabilizing transform for <M>X.</M></EXR>
</DEST>
...
<D>\forall \v \ell\in\rr^k~~\v\ell' \v X_n\toD \v \ell'\v X.</D>
</THM>
<PF>
