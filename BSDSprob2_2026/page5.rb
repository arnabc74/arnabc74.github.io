...
<HEAD1>Monotone and dominated convergence theorems</HEAD1>
We would have been very happy, had there been a result saying: Whenever <M>X_n\toA X</M>  we have <M>E(X_n)\to E(X)</M>. Unfortunately,
 this is not true in general. For one thing we need <M>E(X_n)</M>  and <M>E(X)</M>  to exist. But even if they do, <M>E(X_n)</M> 
 need not converge to <M>E(X).</M>

<EXM>Construct an example where <M>X_n\toA X</M> and <M>E(X_n), E(X)</M>  are all finite, but <M>E(X_n)\not\to E(X).</M>
<SOLN/>
Consider <M>Unif(0,1)</M>
  probability space with <M>X_n(\omega)
 =<CASES>n<IF>\omega\in(*(0,[[1n]])*)</IF> 0<ELSE/></CASES></M>  and <M>X\equiv 0.</M>
<CIMG web="boxexp.png">Graph of <M>X_3</M></CIMG>
Here <M>X_n(\omega)\to X(\omega)</M>  everywhere. 

Let's find <M>E(X_n).</M>  Here
 <D>X_n=<CASES>n & \mbox{ w.p. } & [[1n]]\\0& \mbox{ w.p. } & 1-[[1n]]</CASES>.</D> 
So <M>E(X_n) = n\times [[1n]] + 0\times (*(1-[[1n]])*) = 1.</M> 
 
Since <M>E(X) = 0,</M>  hence <M>E(X_n)\not\to E(X).</M>
</EXM>
So
 we search for extra conditions under
 which <M>E(X_n)\to E(X)</M>  will hold. The following two theorems each provide one such extra condition. 
...
A sequence <M>(X_n)</M>  satisfying the condition <Q> <M>\forall n~~|X_n|\leq Y</M>  for some <M>Y</M>
 ...
 is said to be <TERM>dominated</TERM>  by <M>Y.</M>  Hence the name dominated convergence theorem.
<COMMENT><PF>
Clearly, <M>|X|\leq Y.</M>
...
<EXR>(DCT) Let <M>X</M>  be any random variable. Show that <M>f(t) = E(\sin(tX))</M>  is a continuous function.
...
In all these cases we can talk about the limit of the sequence (even if the limit is not finite). But there are sequences
 for which limit does not make sense.
<CIMG web="dots2.png">The sequence <M>(-1)^n(*(1-[[1n]])*)</M></CIMG>
Here the even terms of the sequence is approaching 1, while the odd terms are approaching <M>-1.</M>  It is
 as if the sequence has
