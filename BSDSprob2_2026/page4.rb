...
<THM name="Continuous mapping theorem (for a.s. convergence)">
Let <M>X_n,X</M>  be all random variables defined on a common probability space. Let <M>f:\rr\to\rr</M>  be any continuous
 function.  If <M>X_n\toA X</M>, then <M>f(X_n)\toA f(X).</M> 
</THM>
<PF>Direct.</PF>
...
<EXR>
Let <M>(X_n)</M>  be iid <M>Poi(\lambda).</M>  Let
<M>Y_n = [[1n]]\sum_1^n (X_k-1)^2.</M>  Show that <M>(Y_n)</M>  converges a.s. Find the limit. 
</EXR>
...
<THM name="Continuous mapping theorem (for convergence in probability)">
Let <M>X_n,X</M>  be all random variables defined on a common probability space. Let <M>f:\rr\to\rr</M>  be any continuous
 function.  If <M>X_n\toP X</M>, then <M>f(X_n)\toP f(X).</M> 
</THM>
<PF>Direct.</PF>
...
<EXR>Let <M>X_n\toP X</M>  (random sequence) and <M>a_n\to a</M>  (fixed sequence). Show that <M>X_n+a_n\toP X+a.</M></EXR>
<EXR>Let <M>X_n\toP X</M>  (random sequence) and <M>a_n\to a</M>  (fixed sequence). Show that <M>a_nX_n\toP aX.</M></EXR>

<EXR>If <M>X_n\toP X,</M>  show that <M>|X_n|\toP |X|.</M></EXR>

<EXR>If <M>X_n\toP X,</M> and <M>f:\rr\to\rr</M>  is a continuous function, then  show that
...
If we define <M>\|X\|_p = (#(E(|X|^p)#)^{[[1p]]},</M>  then <M>\|\cdot\|_p</M>  behaves like "distance from origin",
i.e., 
<UL>
<LI> <M>\|X\|_p \geq 0.</M></LI>
<LI>For any <M>a\in\rr</M>  we have <M>\|aX\|_p = |a|\cdot \|X\|_p.</M></LI>
<LI><M>\|X+Y\|_p \leq \|X\|_p+\|Y\|_p.</M>  This holds if <M>p\geq 1.</M></LI>
</UL>
More technically, <M>\|\cdot\|_p</M>  is called a <TERM>norm</TERM>  when <M>p\geq 1.</M>  

Using this norm-notation, we can define <M>X_n\toL p X</M>  as simply <M>\|X_n-X\|_p\to0.</M>
...
<PF>Shall not prove in this course.</PF>
<COMMENT><PF>
Let <M>Y_n = X_n-X.</M> 
...
The second property follows from the norm properties of <M>\|\cdot\|_p.</M>.</PF>

However, even if <M>X_n\toL p X</M>  and <M>Y_n\toL p Y</M>  and <M>X_nY_n, XY</M>  have finite <M>p</M>-th
 moments, we may have <M>X_nY_n\not\toL p XY.</M>

The first exercise below provides  a counterexample. Also, we do not have a continuous mapping theorem for <M>L_p</M> convergence
 (unless we impose additional conditions).
...
<EXR>Let <M>P(X_n=n) = 1-P(X_n=0)=[[1][n^2]].</M>  Determine for which values of <M>p\geq 1</M>  we have <M>X_n\toL p 0.</M></EXR>

<EXR>Show that if <M>(X_n)</M>  is an iid sequence with mean <M>\mu</M>  and variance
 <M>\sigma^2<\infty,</M>  then <M>[[1n]]\sum_1^n X_k\toL 2 \mu</M>.</EXR>

<EXR>Construct <M>(X_n)</M>  such that <M>X_n\toL 1 X</M>  but <M>X_n\not\toL 2 X</M>.</EXR>
...
Since the behaviour of a random variable is captured completely by its distribution, this is quite a natural definition,
 ...
 F(x)</M> "? Why relax the requirement at the  discontinuity points of <M>F?</M>  Let us see the reason with an example.
To appreciate the discussion better, you need to recall the following graph from Probability 1:
<CIMG web="unifabcdf.png">Distribution function of <M>Unif(a,b)</M></CIMG>

<EXM>Suppose that <M>X_n\sim Unif(*(0,[[1n]])*).</M>  Intuitively, what distribution does <M>(X_n)</M>  approach? 
<SOLN/>
Intuitively the limiting distribution should be degenerate at <M>0</M>, i.e., <M>X_n\toD X,</M>  where <M>X\equiv 0.</M>

Now let us consider the distribution functions of <M>X_n</M>  and <M>X</M>:
<CIMG web="cdfs.png"></CIMG>
It might help to see an animation of the convergence of the <M>F_n</M>'s:
<VID src="cdfs.webm">Click to animate</VID>  
Notice that we have <M>F_n(x)\to F(x)</M>  whenever <M>x\neq 0.</M>  But at <M>x=0,</M>  we have <M>F_n(0)\equiv 0</M>  while
 <M>F(0) = 1.</M>  So <M>F_n(0)\not\to F(0).</M>  

So to make the mathematics behave intuitively, we need to relax the convergence requirement at the
 discontinuity point of
<M>F.</M>  
</EXM>

While convergence in distribution is a natural concept, you should be aware of one pitfall that is illustrated in the next
 example.

<EXM>Consider a <M>Unif(0,1)</M>  probability space, and let <M>X_n(\omega) = \omega.</M>  Also
 let <M>X(\omega) = \omega.</M>  Then obviously <M>X_n\toD X</M>, because they are all actually
 the same random variable! But is the following statement also true: <M>X_n\toD 1-X</M>?
<SOLN/>
While it might surprise you, the answer is "Yes!", because <M>X</M>  and <M>1-X</M>  both have the same distribution, viz.,
 <M>Unif(0,1).</M>  

When we say <M>X_n\toD X</M>,  we actually mean 
<Q>"distribution of <M>X_n</M>  <M>\to</M>  distribution of <M>X</M> ".</Q>
Since distribution of <M>X</M>  is the same as as the distribution of <M>1-X,</M>  convergence in distribution does not distinguish
 between <M>X</M>  and <M>1-X.</M>  

Indeed, a more reasonable notation is <M>X_n\to Unif(0,1).</M>  Here we are writing the
 distribution itself after the arrow, and hence there is no need to use the <M>\toD</M>  notation. 
</EXM>
...
<PF>Direct.</PF> 
However, we do not have any result like "If  <M>X_n\toD X</M>  and <M>Y_n\toD Y</M>, then <M>X_n\pm Y_n\toD X\pm Y.</M>"
 
The following example explains why.
<EXM>
Show that <M>X_n\toD X</M>  and <M>Y_n\toD Y</M>  do not imply <M>X_n+ Y_n\toD X+ Y.</M>
<SOLN/>
...
<THM name="Continuous mapping theorem (for convergence in distribution)">
Let <M>X_n,X</M>  be all random variables (not necessarily defined on a common probability space). Let
 <M>f:\rr\to\rr</M>  be any continuous
 function.  If <M>X_n\toD X</M>, then <M>f(X_n)\toD f(X).</M> 
</THM>
<PF>Skipped.</PF>
...
<PF>Skipped.</PF>
 ...
 distribution of the <M>X_i</M>'s be, if you average a large number of <M>X_i</M>'s you get a close
...
We shall assume that the trials are iid. <HIDE lab="Details">Actually the trials are neither independent nor identially distributed,
 because we typically 
sample
 <I>without replacement</I>  in such a scenario. So the first
 member of the sample was drawn from the entire population, while
the second member was drawn from the <I>rest of the
 population</I>.

However, since the population is large (much
...
or 
<D>\sum_1^{500} X_i \stackrel{\bullet}{\sim} N(0.40\times 500,0.24\times 500)\equiv N(200, 120).</D>
Now we can find the required probability as
...
We have learned about four different modes of convergence. It is surprising
...
<HEAD2><M>a.s.</M>   implies <M>P</M></HEAD2>
Here we shall use the fact that if for a sequence of events <M>(A_n)</M>  we have <M>P(A_n\io)=0</M>  then <M>P(A_n)\to 0.</M> 
 We had seen this in the <LINK to="bc.html#io">Borel-Cantelli page</LINK>. 
...
<HEAD2><M>L_p</M>  implies <M>P</M></HEAD2> 
...
<HEAD2><M>a.s</M>  does not imply <M>L_p</M></HEAD2>
...
Convergence in distribution obviously cannot imply convergence in probability, because
 ...
 space, we can get a counterexample, as shown in the next problem.
<EXR>
 Let <M>U\sim Unif(0,1).</M>  Let <M>X_n = (-1)^n U.</M>  Then
