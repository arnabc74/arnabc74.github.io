...
<COMMENT>The motivation is again from statistical regularity. If you take many IID replications of 
...
Let <M>\v X</M>  be a random vector with components <M>X_1,...,X_n.</M>  Then its <TERM>dispersion matrix</TERM>  or <TERM>variance matrix</TERM> 
 or <TERM>variance-covariance matrix</TERM>  is defined as the <M>n\times n</M>  symmetric matrix
 <M>V(\v X)</M>  with <M>(i,j)</M>-th entry
...
<D>V(X) = E(X-E(X))^2\mbox{ and } \cov(X,Y) = E(X-E(X))(Y-E(Y)).</D>  
If we mimick this definition as follows in the multivariate case, we get the definition of covariance matrix:
<D>V(\v X) = E[#[(\v X-E(\v X))(\v X-E(\v X))']#].</D>
The following example demonstrates this in the 2D case.
<EXM>
Let <M>\v X = <MAT>X_1\\X_2</MAT>.</M>  Then 
<D>\v X-E(\v X) = <MAT>X_1-E(X_1)\\X_2-E(X_2)</MAT>.</D>
So 
<MULTILINE>
& & (\v X-E(\v X))(\v X-E(\v X))'\\
& = & <MAT>X_1-E(X_1)\\X_2-E(X_2)</MAT><MAT>X_1-E(X_1) & X_2-E(X_2)</MAT> \\
& = & <MAT>(X_1-E(X_1))^2 & (X_1-E(X_1))(X_2-E(X_2))\\
(X_1-E(X_1))(X_2-E(X_2)) & (X_2-E(X_2))^2</MAT>.
</MULTILINE>
...
<THM>
Let <M>X,Y</M>  be random variables defined on the same probability space. Let them have finite second moments. Then 
<OL><LI><M>\cov(X,Y) = \cov(Y,X)</M>.</LI>
<LI><M>V(X) = \cov(X,X)</M>.</LI>
<LI>For fixed <M>a,b\in\rr</M>  we have <M>\cov(aX,bY) = ab\,\cov(X,Y).</M></LI>
<LI>For fixed <M>a\in\rr</M>  we have <M>V(aX) = a^2V(X).</M></LI>
</OL>
</THM>
<PF> Direct application of the definition.</PF>

<THM>
If <M>X,Y</M>  are two random variables (both defined on the same probability space) and <M>a,b,c,d\in\rr</M>  are fixed
 numbers, then 
<D>\cov(aX+bY,cX+dY) = ac\,V(X)+(ad+bc)\,\cov(X,Y)+bd\, V(Y).</D>
</THM>
<PF>
Notice the similarity of this result with usual algebraic multiplication that we learn at school:
<D>(ax+by)(cx+dy) = ac\,x^2+(ad+bc)\,xy+bd\, y^2.</D>
Thus, covariance plays the role of multiplication (and so <M>V(X) = \cov(X,X)</M>  plays the role of squaring). Indeed, that
...
<EXR>If <M>X_1,...,X_n</M>  are IID with mean 2 and variance 5, and <M>\v X = (X_1,...,X_n)'</M>,
 then find <M>E(\v X)</M>  and <M>V(\v X).</M> Express <M>E(\v X)</M>  in terms of <M>\v
 1_n</M>, the <M>n\times 1</M>  vector of all <M>1</M>'s. Also express <M>V(\v X)</M>  as a
...
<HINT>
The correlation between two random variables <M>X</M>  and <M>Y</M>  is defined as 
<D>\cor(X,Y) = [[\cov(X,Y)][\sqrt{V(X)V(Y)}]].</D>
</HINT>
</EXR>
<COMMENT>
eigen(matrix(c(3,2,1,2,4,2,1,2,5),3))$val
</COMMENT>
<EXR>In the last problem, also find <M>V(X_1-3X_2)</M>  and <M>\cov(X_1+X_2,X_3).</M></EXR>

<EXR>If <M>X_1,...,X_n</M>  are iid <M>Poi(1),</M>  then find <M>V(\v X),</M>  where<M>\v X = (X_1,...,X_n).</M></EXR>
...
<EXR>Let <M>\v X = (U,U^2),</M>  where <M>U\sim Unif(-1,1).</M>  Find <M>V(\v X).</M></EXR>
...
A real, symmetric matrix <M>A_{n\times n}</M>  is called <TERM>nonnegative definite (nnd)</TERM>  if 
<M>\forall \v\ell\in\rr^n~~\v\ell'A\v\ell\geq 0.</M>

A real, symmetric matrix <M>A_{n\times n}</M> is called <TERM>positive definite (pd)</TERM>  if <M>\forall</M> nonzero 
...
A real matrix <M>A</M>  is pd if and only if <M>A = BB'</M>  for some <I>nonsingular</I> matrix <M>B.</M>
</THM>
...
<HEAD2>Problem set <PS/></HEAD2>
<EXR>If <M>V(\v X_{n\times 1}) = \Sigma,</M>  and <M>\v b</M>  is a fixed
 <M>n</M>-dimensional vector, then what will be the dispersion matrix of <M>\v X+\v b</M>?</EXR>

<EXR>Let <M>\v X = <MAT>X_1\\X_2</MAT>.</M>  If <M>V(\v X) = \Sigma,</M>  
then find <M>\cov(X_1+2X_2,3X_1-5X_2)</M>  in terms of <M>\Sigma.</M></EXR>

<EXR>If <M>V(\v X_{n\times 1}) = \Sigma,</M>  and <M>\v a</M>  and <M>\v b</M>  are fixed
...
<EXR>If <M>V(\v X_{n\times 1}) = \Sigma,</M>  and <M>A_{r\times n}</M>  and <M>B_{s\times n}</M>  are fixed
...
<EXR>If <M>V(\v X) = \Sigma,</M>  and <M>\v a</M>  and <M>\v b</M>  are fixed
...
<EXR>Let <M>V(\v X)=\Sigma</M>  be a pd matrix with  Cholesky square root <M>A.</M> What is the
 dispersion matrix of <M>(A ^{-1})'\v X</M>? </EXR>
...
This is a simplified version of a classical machine learning application. You start with a
 database of face images. (You may use <LINK to="grayfaces.tar.gz">this one</LINK>. Or, if you are
 really ambitious, you may
 take photos of your classmates). Then compute the  covariance matrix of the data, and reduce the data to just
 a few numbers (say 10 to 15) per face. These numbers store most of the "relevant" information for
 recognising the face.  Formally, this project is about dimensionality reduction using principal component analysis. Requires
