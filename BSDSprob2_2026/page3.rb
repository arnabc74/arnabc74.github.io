...
Well, the Cauchy-Schwartz inequality guarantees precisely that. In this geometric interpretation,
 <M>\cos ^2 \theta</M>  is called the correlation coefficient between <M>X</M>  and <M>Y.</M>  
 
<HEAD1>Markov</HEAD1>
<EXM>If average salary in a company is 50,000, then no more than 20% of the employees can make
 250,000 or more. Since 50000/250000 = 1/5.</EXM>

<THM name="Markov inequality">
Let <M>k\in\nn.</M>  Let <M>X</M>  be a nonnegative random variable with <M>E(X^k) < \infty.</M>  
Let <M>\epsilon>0.</M>
Then <M>P(X \geq \epsilon) \leq [[E(X^k)][\epsilon^k]].</M> 
</THM>
<PF>
...
Will higher values of <M>k</M>  necessarily give you lower bounds?

A direct application of this proves the following important result.

<THM>If <M>X_n\to X</M>  in <M>L_1</M>  then <M>X_n\toP X.</M></THM>
<PF>
Take any <M>\epsilon>0.</M>  Then <M>P(|X_n-X|>\epsilon)\leq [[E(|X_n-X|)][\epsilon]]\to 0.</M>
</PF>
An interesting application is in computer science. There is a class of algorithms called Las Vegas algorithms. Such algorithms
approach a problem in a random way that is sure to arrive at the answer, but after a random number
 of steps.  If <M>X</M>  denotes the number steps,  we often know <M>E(X)</M>  as a function of the input size. For instance,
 in the randomised quicksort algorithm <M>E(X) = n\log n,</M>  where <M>n</M>  is the number of entries to be sorted.
...
<EXR>Prove that <M>P(X>10E(X))\leq [[1][10]].</M></EXR>  
...
A router receives a random number (<M>X</M>) of packets every millisecond. We know from the
 ...
 hold 250 packets before overflowing. The network engineer needs to estimate the failure rate,
 which is <M>P(X\geq 250) \leq 20\%,</M>  by the Markov inequality.
</EXR>
...
A bank has a large portfolio of small business loans. <M>X=</M> number of defaulters in a
 ...
 20M.</M>  Bound probability of that.
</EXR>
<EXR>A financial system tracks <M>X</M>  the downtime of the system per year. Past data shows
 <M>E(X)=15</M>  minutes. Crash would result if <M>X\geq 2</M>  hours. Bound its probability.</EXR>
<EXR>
During highly volatile market anomaly, an investment fund's historical data becomes useless.
 The only certain information is the current daily expected loss <M>E(X)=100,000.</M>  Upper bound <M>P(X\geq 1,000,000).</M>
</EXR>
<LI>A randomised primality testing algorithm called the Miller Rabin test applies a random
 ...
 But if it says "Prime" then the input is composite with 0.25 probability. If this experiment is
 run <M>k</M>  times independently, and we get "Prime" every times, then the cance that number is
 actually composite is <M>\leq 0.25^k.</M>
<M>X</M> 
 errors during a single test run. By design <M>E(X)=0.25,</M>  say. We shall run this test run
 independently <M>k</M>  times. Bound the probability of failure, <M>P(X\geq 1)^k\leq 0.25^k.</M> </LI>
<TODO>
Application areas:
<UL>
<LI>In network and queueing theory: estimate the probability that a packet delays or queue lengths
 will exceed a critical threshold. No distributional modelling needed.
</LI>
<LI>risk management and finance: wors-case estimates of extreme financial losses or insurance
 claims relaying solely on average historical risk.</LI>
</UL>
</TODO>


<HEAD2>Count-min sketch</HEAD2>
Imagine a stream of items going past you (like cars of different brands or trees of different
 ...
 computer connected to the internet). Your job is to keep track of the frequencies of the
 different  types you have seen so far. Tally marking is a very simple technique to achieve this.
...
<TODO>
<OL>
<LI>Distinct elements counting like in the BJKST algo: See pdf downloaded in mobile.</LI>
<LI>Frequency estimation (like count-min sketch): See pdf downloaded in mobile.</LI>
<LI>Machine learning and PAC learning: Mention PAC as probability convergence.</LI>
</OL>
</TODO>
...
A point about the inequalities in the above theorem. There are two inequalities, one inside the probability, and one outside.
 ...
 get <M>0 < 0</M>  for a degenerate <M>X</M>.
<HEAD1>WLLN</HEAD1>
...
<HEAD1>Strong law of large numbers (SLLN)</HEAD1>
