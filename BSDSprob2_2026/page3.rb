...
<EXM>Average salary in a company is Rs 50,000. It is claimed that 30% of the employess are getting
 a salary of Rs 250,000
 or more. Can this claim be true?<SOLN/>
...
In fact, no more than 20% of
 the employees can make
 250,000 or more. Since 50000/250000 = 1/5.</EXM>

This commonsense notion leads to the following very useful inequality.
...
<EXR>Prove that <M>[[1][\sqrt{2\pi]]\int_{-\infty}^ \infty e^{-x^2/2}\, dx \leq [[1][\epsilon^4]].</M></EXR>

The following important result is a direct corollary to Markov inequality. 
...
Markov inequality is little more than commonsense, and naturally it finds many practical applications. Its greatest strength
 lies in the fact that it makes no use of the distribution of the the random variable other than through its expectation.
For many real life random variables, estimating the mean is far easier than estimating the entire distribution. 

<HEAD3>Analysing Las Vegas algorithms</HEAD3> 
...
One example is the randomised quick sort algorithm. Here we sort an array of numbers by picking a random element from the
 araay, and dividing the rest into two groups, depending on whether a number is <M>\leq</M>  or <M>></M>  than the picked
 element. Then we to the same things recursively to both the groups separately. 

Let <M>X</M>  denote the number steps in a Las Vegas algorithm.   We often know <M>E(X)</M>  as a function of the
 input size (under suitable assumptions on the input). For instance,
...
<HEAD3>Chance of liquidity crisis</HEAD3>
<EXR>
...
<HEAD3>Chance of too long system downtime </HEAD3>
...
<HEAD3>Primality testing</HEAD3>
<EXR>A randomised primality testing algorithm called the Miller Rabin test applies a random
...
Give an uppoer bound on the probability that this battery of <M>k</M>  tests makes an error. [This problem can be solved
 merely by commonsense, without explicit use of Markov inequality, which by the way is little more than commonsense itself!]
