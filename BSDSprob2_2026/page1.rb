...
For example, our <M>\phi</M>  has component functions <M>\phi_1(x_1,x_2) = x_1,</M>  <M>\phi_2(x_1,x_2) = x_1+x_2</M>  and
...
Let <M>\phi:\rr^n\to\rr^m</M>  be a function with <M>i</M>-th component function denoted by
 <M>\phi_i(x_1,...,x_n).</M> Assume that <M>\pd {\phi_i}{x_j}</M>  is defined for all <M>i,j.</M> 
 Then the Jacobian matrix of <M>\phi</M>  is defined as the <M>m\times n</M>  matrix with <M>(i,j)</M>-th entry 
<D>\pd{\phi_i}{x_j}.</D>
<EXM>Find Jacobian of <M>\phi(x_1,x_2) = (x_1,x_1+x_2,x_1^2)</M><SOLN/>
The Jacobian is going to be a <M>3\times 2 </M> matrix:
<D><MAT>1 & 0\\1 & 1\\2x_1 & 0</MAT>.</D>
</EXM>
 <HEAD2>Problem set</HEAD2>
<HEAD1>Review of the univariate case</HEAD1>
...
In higher dimensions, we want to have <M>\phi:\rr^n\to\rr^n.</M>  Now, if <M>n\geq 2,</M>  then 
...
<THM>
Let <M>\v X</M>  be an <M>\rr^n</M>-valued random vector. Let <M>\phi:\rr^n\to\rr^n</M>  be a
bijection with differentiable inverse. Let <M>\v Y = \phi(\v X).</M> 
...
Clearly, <M>\phi:S\to \phi(S)</M>  is a bijection,  because given <M>y_1=x_1x_2</M>  and <M>y_2=x_1</M>   you can recover <M>(x_1,x_2)\in[1,2]\times[3,4]</M> 
 uniquely. 
...
If our transformation is <M>\phi:\rr^n\to\rr^n</M>  is of the form <M>\phi(\v x) = A\v x + \v b,</M>  where <M>A_{n\times n}</M> 
 is a fixed matrix, and <M>\v b</M>  is a fixed vector, then <M>\phi</M>  is a a bijection if any only if <M>A</M>  is nonsingular.
 In this case, the Jacobian of <M> \phi ^{-1}</M>  is <M>A ^{-1}.</M>  So, if the density of <M>\v X</M>  is <M>f(\v x),</M> 
 then density of <M>\v Y</M>  is <M>g(\v y) = [[1][|A|]] f( A ^{-1} (\v x - \v b)).</M>

<HEAD2>Problem set</HEAD2>
<HEAD1>Transformations from <M>\rr^n</M>  to <M>\rr^m</M></HEAD1>
So far we have been talking only about trasnformas from <M>\rr^n</M>  to <M>\rr^n.</M>  What if we have a transform <M>\phi:\rr^n\to\rr^m,</M> 
 where <M>m\neq n?</M>  
...
But, it is quite common to have the <M>m < n</M>  case. In this case, the Jacobian formula is not applicable directly. However,
 we can "pad up" <M>\phi</M>  to make the codomain equal to <M>\rr^n</M>  keeping the padded function bijective. Here is
 an example. 

<EXM>We are given the joint density of <M>(X,Y)</M>  and want to find the joint density of
 <M>2X+3Y.</M>  How can you "pad it up" and use the Jacobian formula?<SOLN/>
Here the function is <M>\phi(x,y) = 2x+3y.</M>  Since the codomain is <M>\rr,</M>  we make it
 <M>\rr^2</M>  by padding it with another
 function of our choice, say <M>\xi(x,y) = x.</M>  Then the padded function is <M>(x,y)\mapsto (2x+3y,x).</M>  This is a
bijective function from <M>\rr^2</M>  to <M>\rr^2.</M>  So we can let <M>V = X,</M>  and find the joint density of
 <M>(U,V)</M>  from that of <M>(X,Y)</M>  using the Jacobian formula. Then we can integrate out <M>V</M>  to find the marginal
 density of <M>U.</M>
</EXM>
...
The above picture shows the effect of the function <M>\phi(x_1,x_2)=(#( (2x_1+x_2)^2,\, (x_1-x_2)^3)#)</M> 
...
We want to measure which square will grow and which square will be squeezed. For this we consider
 some point <M>(a,b)</M>   in the domain, and a little square <M>[a,a+\delta x]\times[b , b+ \delta y].</M>  
...
The blue edge is given by the vector <M>(*(\pd u x, \pd v  x  )*)\delta,</M>  and red side by the vector
 <M>(*(\pd u y, \pd v  y )*)\delta.</M>

Now, it is known that the area of a parallelogram with sides given by <M>(a,b)</M>  and
 <M>(c,d)</M>  is <M>[|[\det<MAT>a & c\\b & d</MAT>]|].</M>
Hence the area of the green parallelogram is 
<D>[|[ \det<MAT>\pd u x & \pd u y\\\pd v x & \pd v y</MAT>]|] \delta^2,</D>
evaluated at <M>(a,b).</M>  Since the original green square had area <M>\delta^2,</M>  hence the scaling factor is given
 by the absolute determinant of the Jacobian.

 into the curve of <M>\phi(a,y)</M>  and blue line changes into the curve of <M>\phi(x,b).</M> 
 When <M>x</M>  increases from <M>a</M>  to <M>a+\delta x,</M>  the corresponding change in
 <M>\phi(x,b)</M>  is from <M>\phi(a,b)</M>  to  
...
<Q><ALERT/>If all this looks like unmotivated magic, you might benefit from 
<LINK  to="https://youtu.be/zm3L8Dw79xA?si=IVz7PWwway-myQ3R">this introductory  video
...
<HEAD2>Problem set </HEAD2>
<EXR>Compute the Jacobian matrix for <M>\phi(x,y) = (x+y,x-y).</M></EXR>
