...
<UL><LI><B>Substep 1:</B>  We can show that for any non-negative random variable, <M>Z</M>, we have an non-decreasing 
 sequence of simple random variables <M>(W_n)</M>  such that <M>W_n\nearrow Z.</M></LI>
<LI><B>Substep 2:</B>  We can show that if we have any sequence <M>(W_n)</M>  and any nonnegative random
 variable <M>Z</M>  with <M>W_n\nearrow Z</M>, then <M>E(W_n)\to Z.</M></LI>
<LI><B>Substep 3:</B>  We apply the above two substeps to both <M>X</M>  and <M>Y</M>  to obtains
 non-decreasing sequences <M>(U_n)</M>  and <M>(V_n)</M>  such that <M>E(U_n)\to E(X)</M>  and
 <M>E(V_n)\to E(Y).</M>  Since <M>U_n+V_n\nearrow X+Y</M>, the result follows from the simple case.</LI>
</UL> 
</PF>
