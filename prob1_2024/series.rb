...
<UL>
<LI>If this limit exists, then we say that the series <B>converges</B> to
that value. An example is <M>1+[[12]]+[[1][2^2]]+[[1][2^3]]+\cdots = 2.</M>
</LI>
<LI>If the limit is <M>\infty </M> or <M>-\infty </M>
then we say that the series <B>diverges</B> to <M>\infty </M>
or <M>-\infty.</M> A trivial example is <M>1+1+1+\cdots = \infty.</M>
</LI>
<LI>It is possible that the series neither
converges nor diverges. We then say that the 
series <B>oscillates</B>. An example is 
<D>
1-1+1-1+1-1+-+-\cdots.
</D>
...
<THM>
Consider an infinite series <M>a_1+a_2+a_3+\cdots,</M>
where <M>a_i</M>'s are any real numbers
(positive/negative/zero). If the infinite series of the absolute
terms <M>|a_1|+|a_2|+|a_3|+\cdots</M> converges (to some finite
limit), then the terms in the original series may be regrouped
and rearranged without affecting the sum.
</THM>

If this condition is met (i.e, the absolute series converges),
then we call the original series <B>absolutely convergent</B>. It
is an interesting and <I>very useful</I> fact that any absolutely
convergent series must also be convergent. 
<P/>
You'll learn the proofs of all these theorems in you analysis
course. 
<P/>
In our probability course We shall  work with <I>only</I> infinite series of nonnegative terms or
