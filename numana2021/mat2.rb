...
<M>x_1,y_1,z_1.</M> Thus,
 <D>
<MAT>x_1\\y_1\\z_1</MAT> = <MAT>(19-3y_0+4z_0)/20\\
(3+x_0+z_0)/4\\
(7-x_0+4y_0)/10
...
0  0          0          0
1  0.95       0.75       0.70               
2  0.9775     1.1625     0.9050         
3  0.956625   1.220625   1.067250   
4  0.9803563  1.2559688  1.0925875
5  0.9801222  1.2682359  1.1043519
6  0.980635   1.271119   1.109282   
7  0.9811887  1.2724793  1.1103839
8  0.9812049  1.2728931  1.1108728
9  0.9812406  1.2730194  1.1110368
10 0.9812544  1.2730693  1.1110837
</PRE>
...
<THM>For all initial values, a necessary and sufficient condition for <M>\be_n\to \bz</M> is that 
...
the Gauss-Jacobi method must converge for all initial values.
</THM>
<PF>
...
<THM>
The Gauss-Seidel method converges for all initial values if the coefficient matrix is
positive definite.
</THM>
<PF>
...
a symmetric <M>2\times 2</M> matrix. The following exercise gives
the details.

<EXR>Show that if <M>\tan(2 \theta ) =
[[2b][c-a]]</M>, then the off-diagonal entries
of <M>G(\theta) <MAT>a & b\\b & c</MAT> G(\theta)'</M> are zero.
</EXR>



We shall often need to apply the transform to only a <M>2\times 2</M> submatrix
in an <M>n\times n</M> matrix, <M>A.</M> If  we are working with
its <M>\{i,j\}\times \{i,j\}</M> submatrix (where <M>i\neq j</M>),  we first compute the
 Given's matrix, <M>G,</M> for the submatrix, and define
an <M>n\times n</M> matrix <M>G(i,j,A)</M> 
which is the same as <M>I_{n\times
n}</M> except that its <M>\{i,j\}\times \{i,j\}</M> submatrix
replaced by <M>G.</M> For example, 
if 
<D>
G = <MAT>p & q\\r & s</MAT>,
</D>
...
has the <M>(i,j)</M>-th entry equal to <M>0.</M> 
the left changes
<I>only</I> the 2nd and 4th rows of the matrix. Similarly,
...
the <M>(i,j)</M>-th entry (with <M>i> j</M>). Then we construct the matrix 
<M>G = G(i,j,a_{ij}, a_{jj}).</M>

<P/>

Then it may be easily checked
that <M>GAG'</M> has the <M>(i,j)</M>-th
...
off-diagonal entries in a symmetric matrix <M>A.</M> Let <M>i\neq
j</M> and <M>B=G(i,j,A) A G(i,j,A)'.</M> Show that <M>f(B)
= f(A) - 2a_{ij}^2.</M></EXR>

<EXR>Let the <M>(i,j)</M>-th entry of <M>A</M> be the largest
absolute off-diagonal entry of <M>A.</M>  Let <M>B=G(i,j,A)
A G(i,j,A)'.</M> Show that <M>f(B)\leq(*(1-[[1N]])*) f(A)</M>, where <M>2N</M> is the number of
...
<DISQUSE id="mat2" url="https://arnabc74.github.io/numana2021/mat2.html"/>
@}
</NOTE>
