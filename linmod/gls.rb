...
correlated with correlation <M>0.5.</M>
Also <M>\epsilon_{ij}</M>'s all have the same unknown
variance <M>\sigma^2.</M> Suggest how you may
obtain MLE of <M>\mu</M> under normality assumption.
</LI>

<LI>Consider the last problem again. Let the correlation
between <M>\epsilon_{i1}</M> and <M>\epsilon_{i2}</M> be some
known number <M>r\in[0,1].</M> Let the MLE
of <M>\sigma^2</M> be  <M>\h \sigma^2(r).</M> Would you
expect <M>\h \sigma^2(r)</M> to be an increasing function
of <M>r</M> or a decreasing function?</LI>
</OL>
